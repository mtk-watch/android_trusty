...
#
# Modification based on code covered by the original mentioned copyright
...
make-make:
	@PROJECT=$(project-name) PROJECT_MK=$(project-name) $(MAKE) -rR -f $(LOCAL_MAKEFILE) $(filter-out $(project-name), $(MAKECMDGOALS))

.PHONY: make-make
else
ifneq ($(TARGET_BOARD_PLATFORM),)
TRUSTY_PLATFORM=$(TARGET_BOARD_PLATFORM)
else
TRUSTY_PLATFORM=$(shell echo $(MTK_PLATFORM) | tr A-Z a-z)
endif

ifneq ($(strip $(foreach d,$(LKINC),$(wildcard $(d)/project/$(TRUSTY_PLATFORM).mk))),)
do-nothing := 1
$(MAKECMDGOALS) _all: make-make
make-make:
	@PROJECT=$(project-name) PROJECT_MK=$(TRUSTY_PLATFORM) $(MAKE) -rR -f $(LOCAL_MAKEFILE) $(filter-out $(project-name), $(MAKECMDGOALS))
...
$(info CFG_LOG_REG_BASE = $(CFG_LOG_REG_BASE))
$(info CFG_LOG_BAUDRATE = $(CFG_LOG_BAUDRATE))
